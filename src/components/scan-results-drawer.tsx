...
export function ScanResultsDrawer({
  ...
  const fixDef = AUTO_FIXES[effectiveKey];
      ...
      <SheetContent className="w-full sm:max-w-lg p-0">
        ...
        <ScrollArea className="h-[calc(100vh-5rem)]">
          <div className="px-6 py-5 space-y-5">
                      ...
                      <div key={i} className="space-y-1">
                        ...
                        {finding.pageUrl && (
                          <p className="text-xs text-muted-foreground ml-5">
                            Found on: <a href={finding.pageUrl} target="_blank" rel="noopener noreferrer" className="underline">{finding.pageUrl}</a>
                          </p>
                        )}
                      </div>
                    ))}
            ...
            {/* Guided fix flow: show FixFlowPanel instead of static analysis + remediation */}
            {isGuidedWithAnalysis && site ? (
              <>
                <Separator />
                <FixFlowPanel
                  checkKey={effectiveKey}
              ...
              <>
                {/* Non-guided analysis results (B1 etc.) */}
