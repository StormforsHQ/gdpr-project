...
export const CHECKLIST: CheckCategory[] = [
  ...
  },
  {
    ...
    checks: [
      ...
      {
        key: "G8", label: "Consent withdrawal accessible at all times", description: "Visitors must be able to change their consent choice at any time. Either a floating cookie widget (Cookiebot's Privacy trigger) or a 'Cookie settings' link in the footer is fine - the floating widget is not a legal requirement, just a convenience. What matters is that the mechanism reopens the consent banner, is as easy as giving consent (GDPR Art. 7(3)), allows granular withdrawal per category, takes effect immediately, and doesn't require login or extra friction.", automation: "browser-manual", tier: "basic", method: "Manual browser check. After making a consent choice, look for a way to change it: a floating cookie widget (usually bottom-left) or a 'Cookie settings' link in the footer. Either one is acceptable. Click it and verify the consent banner reopens with your previous choices shown. If neither exists, mark as issue. The scanner can't automate this because it requires interacting with the live consent mechanism.",
